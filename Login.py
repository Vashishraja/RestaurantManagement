    ...
    if b==c:
        if str(mix) in str(d):
            ...
            def system():
                ...
                # Topframe
                topframe = Frame(root, bg="lightyellow", width=1600, height=50)
                topframe.pack(side=TOP)

                # Menu Frame
                menuframe = Frame(root, bg="lightyellow", width=1600, height=50)
                menuframe.pack(side=TOP)

                # Leftframe
                leftframe = Frame(root, bg="lightyellow", width=900, height=600)
                leftframe.pack(side=LEFT)

                # rightframe
                rightframe = Frame(root, bg="lightyellow", width=400, height=600)
                rightframe.pack(side=RIGHT)

                # bottomframe
                bottomframe = Frame(root, bg="lightyellow", width=400, height=50)
                ...
                def add():
                    ...
                    if orders == "" or pizzas == "" or burgers == "" or ices == "" or drinkss == "" or costs == "" or subtotals == "" or taxs == "" or services == "" or totals == "":
                        ...
                        connectn.commit()
                        messagebox.showinfo("Message", "Ordered Successfully")
                    # refresh table data
                    DisplayData()
                    connectn.close()
                ...
                # Top part
                main_lbl = Label(topframe, font=('Calibri', 30, 'bold'), text="Restaurant Management System",
                                 fg="black", bg="lightyellow",
                                 anchor=W)
                main_lbl.grid(row=0, column=3)
                main_lbl = Label(topframe, font=('Calibri', 15,), text=localtime, fg="black", bg="lightyellow",
                                 anchor=W)
                main_lbl.grid(row=2, column=3)

                ### Labels
                # items
                ordlbl = Label(leftframe, font=('Calibri', 16, 'bold'), text="Order No.", bg="lightyellow", fg="black",
                               bd=5, anchor=W).grid(row=1,
                                                    column=0)
                ordtxt = Entry(leftframe, font=('Calibri', 16, 'bold'), bd=6, insertwidth=4, justify='right',
                               textvariable=orderno).grid(row=1, column=1)
                # Pizza
                pizlbl = Label(leftframe, font=('Calibri', 16, 'bold'), text="Pizza", bg="lightyellow", fg="black",
                               bd=5, anchor=W).grid(row=2,
                                                    column=0)
                piztxt = Entry(leftframe, font=('Calibri', 16, 'bold'), bd=6, insertwidth=4, justify='right',
                               textvariable=pizza).grid(row=2, column=1)
                # burger
                burlbl = Label(leftframe, font=('Calibri', 16, 'bold'), text="Burger", bg="lightyellow", fg="black",
                               bd=5, anchor=W).grid(row=3,
                                                    column=0)
                burtxt = Entry(leftframe, font=('Calibri', 16, 'bold'), bd=6, insertwidth=4, justify='right',
                               textvariable=burger).grid(row=3, column=1)

                # icecream
                icelbl = Label(leftframe, font=('Calibri', 16, 'bold'), text="Ice Cream", bg="lightyellow", fg="black",
                               bd=5, anchor=W).grid(row=4,
                                                    column=0)
                icetxt = Entry(leftframe, font=('Calibri', 16, 'bold'), bd=6, insertwidth=4, justify='right',
                               textvariable=icecream).grid(row=4, column=1)
                # drinks
                drinklbl = Label(leftframe, font=('Calibri', 16, 'bold'), text="Drinks", bg="lightyellow", fg="black",
                                 bd=5, anchor=W).grid(row=5,
                                                      column=0)
                drinktxt = Entry(leftframe, font=('Calibri', 16, 'bold'), bd=6, insertwidth=4, justify='right',
                                 textvariable=drinks).grid(row=5, column=1)
                # cost
                costlbl = Label(leftframe, font=('Calibri', 16, 'bold'), text="Cost", bg="lightyellow", bd=5,
                ...
                # subtotal
                sublbl = Label(leftframe, font=('Calibri', 16, 'bold'), text="Subtotal", bg="lightyellow", bd=5,
                ...
                # tax
                taxlbl = Label(leftframe, font=('Calibri', 16, 'bold'), text="Tax", bg="lightyellow", bd=5,
                ...
                # service
                servicelbl = Label(leftframe, font=('Calibri', 16, 'bold'), text="Service", bg="lightyellow", bd=5,
                                   anchor=W).grid(row=9,
                                                  column=0)
                servicetxt = Entry(leftframe, font=('Calibri', 16, 'bold'), bd=6, insertwidth=4, justify='right',
                                   textvariable=service).grid(row=9, column=1)
                # total
                totallbl = Label(leftframe, font=('Calibri', 16, 'bold'), text="Total", bg="lightyellow", bd=5,
                ...
                totbtn = Button(leftframe, font=('Calibri', 15, 'bold'), text="Total", bg="yellow", fg="black", bd=3,
                                padx=4, pady=4,
                                width=10, command=tottal).grid(row=4, column=3)

                resetbtn = Button(leftframe, font=('Calibri', 15, 'bold'), text="Reset", bg="yellow", fg="black", bd=3,
                                  padx=4,
                                  pady=4, width=10, command=reset).grid(row=6, column=2)

                exitbtn = Button(leftframe, font=('Calibri', 15, 'bold'), text=" Exit ", bg="yellow", fg="red", bd=3,
                                 padx=4,
                                 pady=4, width=10, command=exit).grid(row=10, column=2)

                addbtn = Button(leftframe, font=('Calibri', 15, 'bold'), text="Add", bg="yellow", fg="black", bd=3,
                                padx=4, pady=4,
                                width=10, command=add).grid(row=4, column=2)

                deletebtn = Button(leftframe, font=('Calibri', 15, 'bold'), text="Delete", bg="yellow", fg="black",
                                   bd=3,
                                   padx=4, pady=4, width=10, command=Delete).grid(row=6, column=3)
                ...
                def feedbackk():
                    ...
                    # label#
                    lb1 = Label(feed, font=("Calisto MT", 15, "bold"), text="Thanks for Visiting!", fg="black").pack(
                        side=TOP)
                    lbl2 = Label(feed, font=("calisto MT", 15),
                                 text="We're glad you chose us ! Please tell us how it was!",
                                 fg="black").pack(side=TOP)
                    # name
                    namelbl = Label(feed, font=('vardana', 15), text="Name:-", fg="black", bd=10, anchor=W).place(x=10,
                                                                                                                  y=150)
                    nametxt = Entry(feed, font=('vardana', 15), bd=6, insertwidth=2, bg="white", justify='right',
                                    textvariable=name).place(x=15, y=185)
                    # email
                    emaillbl = Label(feed, font=('vardana', 15), text="Email:-", fg="black", bd=10, anchor=W).place(
                        x=280, y=150)
                    ...
                    checkvar3 = StringVar()
                    checkvar4 = StringVar()
                    c1 = Checkbutton(feed, font=('Calibri', 10, "bold"), text="Excellent", bg="white",
                                     variable=checkvar1)
                    c1.deselect()
                    c1.place(x=15, y=265)
                    ...
                    c3 = Checkbutton(feed, font=('Calibri', 10, "bold"), text=" Average", bg="white",
                                     variable=checkvar3, )
                    c3.deselect()
                    c3.place(x=220, y=265)
                    c4 = Checkbutton(feed, font=('Calibri', 10, "bold"), text="   Poor  ", bg="white",
                                     variable=checkvar4, )
                    c4.deselect()
                    c4.place(x=320, y=265)
                    # comments"
                    commentslbl = Label(feed, font=('Calibri', 15), text="Comments", fg="black", bd=10, anchor=W).place(
                        x=10, y=300)
                ...
                # Feedbackbutton
                feedbtn = Button(menuframe, font=('Calibri', 15, 'bold'), text="Feedback Form", fg="darkblue",
                                 bg="yellow", bd=3, padx=6,
                                 pady=6, width=12, command=feedbackk).grid(row=1, column=1, columnspan=1)
                ...
                # menubutton
                menubtn = Button(menuframe, font=('Calibri', 15, 'bold'), text="Menu Card", bg="yellow", fg="darkblue",
                                 bd=3, padx=6,
                                 pady=6, width=12, command=menu).grid(row=1, column=0)
...
log.geometry("1700x800")
log.title("Welcome To My Page")
